...
3. For every OBS evidence node, check attrs.technique against the crop you opened and against the panel's OCR cue classes (map: /home/aid1/Documents/causalmat/trace_kit/cue_technique.json). Flag every mismatch and fix it (right panel, or drop the citation). Known misfire: the OCR scale-bar detector gives the "micrograph" cue to some plots and maps; when the crop you opened plainly is the right panel for the node and the only conflict is such a false "micrograph" cue, keep or restore the citation and record it under review.cue_overrides [{"node","panel","cue","why"}] . Also check attrs.read_from: an observation that restates an annotation string listed for its panel must be "annotation".
...
6. Only after the graph is final: read v07/{PART}/judge_only/<paper>.matmech.md and record, WITHOUT editing the graph, for each MatMech cause->effect pair whether the graph supports it, contradicts it, or does not cover it.
...
Write graphs_v07/<name>.judge.md: verdict, changes, every panel check, technique mismatches, mode changes, the MatMech tally.
