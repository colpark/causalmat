...
def instructions(jobs_path, parallel="5"):
    ...
    return (f"You are a relay. The file {os.path.abspath(jobs_path)} lists {len(jobs)} jobs, each with an \"agent\" type and a "
            ...
            "content byte for byte: no preamble, no edits, no trimming inside the text. Dispatch at most "
...
def harvest(jobs_path, relay_id, tasks_dir):
    ...
    for j in jobs:
        ...
        cand = [(a, p, i) for a, p, i in D if a == j["agent"] and p.strip() == want and i]
        ...
        for a, p, i in reversed(cand):
            ...
            if reply and reply.strip():
                done = (i, prompt.strip() == want, reply); break
        if not done:
            near = [i for a, p, i in D if a == j["agent"] and p.strip() != want and p.strip()[:200] == want[:200]]
